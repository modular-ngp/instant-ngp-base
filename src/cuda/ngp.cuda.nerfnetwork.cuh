#ifndef INSTANT_NGP_BASE_NGP_CUDA_NERFNETWORK_H
#define INSTANT_NGP_BASE_NGP_CUDA_NERFNETWORK_H
...
namespace ngp::cuda::hidden {
    ...
    public:
        ...
        [[nodiscard]] uint32_t num_forward_activations() const override;

        [[nodiscard]] tcnn::json hyperparams() const override;
        void set_params_impl(T* params, T* inference_params, T* gradients) override;
        void initialize_params(tcnn::pcg32& rnd, float* params_full_precision, float scale) override;
        [[nodiscard]] size_t n_params() const override;
        [[nodiscard]] std::vector<std::pair<uint32_t, uint32_t>> layer_sizes() const override;
        [[nodiscard]] uint32_t input_width() const override;
        [[nodiscard]] uint32_t padded_output_width() const override;
        [[nodiscard]] uint32_t output_width() const override;
        [[nodiscard]] uint32_t required_input_alignment() const override;
        [[nodiscard]] uint32_t width(uint32_t layer) const override;
    ...
    template <typename T>
    NerfNetwork<T>::NerfNetwork(uint32_t n_pos_dims, uint32_t n_dir_dims, uint32_t n_extra_dims, uint32_t dir_offset, const nlohmann::json& pos_encoding, const nlohmann::json& dir_encoding, const nlohmann::json& density_network, const nlohmann::json& rgb_network) : m_dir_offset{dir_offset}, m_n_dir_dims{n_dir_dims}, m_n_extra_dims{n_extra_dims},
...
#endif // INSTANT_NGP_BASE_NGP_CUDA_NERFNETWORK_H
